  ...
  readdirSync(path) {
    ...
    try {
      val = readdirSync(path, { __proto__: null, withFileTypes: true }, {
        encoding: this.#isBuffer ? 'buffer' : 'utf8' });
    } catch {
      val = [];
    }
...
class Glob {
  ...
  constructor(pattern, options = kEmptyObject) {
    ...
    this.#cache.isBuffer = process.platform === 'linux' && encoding === 'buffer';
    if (exclude != null) {
  ...
  #addSubpatterns(path, pattern) {
    ...
    if (isLast && typeof pattern.at(-1) === 'string') {
      ...
      const joinedPath = this.#isBuffer && Buffer.isBuffer(p) ? 
        Buffer.concat([Buffer.from(fullpath), Buffer.from('/'), p]) : 
        join(fullpath, p);
      const stat = this.#cache.statSync(joinedPath);
      if (stat && (p || isDirectory)) {
        const resultPath = this.#isBuffer && Buffer.isBuffer(p) ?
          Buffer.concat([Buffer.from(path), Buffer.from('/'), p]) :
          join(path, p);
        this.#results.add(resultPath);
      }
